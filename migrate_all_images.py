...
from utils import (
    load_json, save_json, IMAGE_KEYWORDS, SECTION_PRESETS,
    FILE_FIELD_PRESETS, ARTICLE_FILE_METAFIELDS, FILE_MAP_FILE,
)
...
def _guess_preset(section_type, setting_key):
...
def _resolve_shopify_image_to_url(client, image_ref):
...
def _extract_template_images(template):
        ...
        for key, value in settings.items():
            if _is_image_setting(key) and value:
                images.append({
                    ...
                    "setting_key": key,
                    "value": value,
                    "preset": _guess_preset(section_type, key),
                })
