...
def update_shopify_images(en_images, ar_images, dry_run=False):
    ...
    if ar_images:
        print(f"  Arabic images stored in metafield: custom.arabic_images")
        print(f"  Theme should check locale and use metafield URLs for Arabic")
    ...
    for sku in en_by_sku:
        ...
        if ar:
            ...
            for u in ar_urls:
                print(f"      {u}")
            if en_urls == ar_urls:
                print(f"    >>> SAME image URLs")
            else:
                print(f"    >>> DIFFERENT image URLs")
        else:
            print(f"    AR: not found")
